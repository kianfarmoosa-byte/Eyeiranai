          ...
          {([
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex items-center gap-2 px-4 py-2.5 text-sm border-b-2 whitespace-nowrap ${tab === t.id ? "border-emerald-600 text-emerald-600 dark:text-emerald-400" : "border-transparent text-slate-600 dark:text-slate-400"}`}>
                ...
                clusters.map(c => (
                  <div key={c.id} className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <button onClick={() => setOpenCluster(openCluster === c.id ? null : c.id)}
                      ...
                      <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-violet-500 to-emerald-500 text-white flex items-center justify-center shrink-0">
                      ...
                      <span className="text-xs px-2 py-1 rounded-md bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 shrink-0">{c.size}</span>
                      {onOpenTimeline && (
                        <span onClick={(e) => { e.stopPropagation(); onOpenTimeline({ kind: "ids", ids: new Set(c.articles.map(a => a.id)), label: c.label }); }}
                          className="p-1.5 rounded-md hover:bg-emerald-100 dark:hover:bg-emerald-900/40 text-slate-400 hover:text-emerald-600 cursor-pointer" title="خط زمانی این خوشه">
                        ...
                        <ol className="relative ms-6 my-2 border-s border-slate-200 dark:border-slate-800">
                          {t.articles.map(a => (
                            <li key={a.id} className="ms-4 mb-2">
                              ...
                              <button onClick={() => onSelect(a.id)} className="text-sm text-right hover:text-emerald-600">
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {(["all", "person", "organization", "place", "thing"] as const).map(k => (
                  <button key={k} onClick={() => setEntityFilter(k)}
                    className={`px-2.5 py-1 rounded-md text-xs ${entityFilter === k ? "bg-emerald-600 text-white" : "bg-slate-100 dark:bg-slate-900"}`}>
                    ...
                    <div key={e.text + e.kind} className="rounded-lg border border-slate-200 dark:border-slate-800 p-2.5 flex items-center gap-2 bg-white dark:bg-slate-950">
                      ...
                      {onOpenTimeline && (
                        <button onClick={() => onOpenTimeline({ kind: "entity", text: e.text, label: e.text })}
                          className="p-1 rounded-md hover:bg-emerald-100 dark:hover:bg-emerald-900/40 text-slate-400 hover:text-emerald-600" title="خط زمانی">
                    ...
                    <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-900 space-y-1">
                      {g.duplicates.map(d => (
                        <button key={d.id} onClick={() => onSelect(d.id)}
                          className="w-full flex items-center gap-2 text-right text-xs hover:text-emerald-600">
