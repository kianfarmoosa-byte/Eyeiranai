...
export function AddFeedDialog({ open, onClose, onDone }: Props) {
  ...
  const addAllIrani = async () => {
    if (!confirm(`افزودن همه ${SEED_FEEDS.length} منبع به خوراک‌های شما؟`)) return;
    setLoading(true);
    try {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-white dark:bg-slate-900 rounded-xl w-full max-w-3xl h-[80vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-800">
          <button onClick={() => setTab('browse')} className={`px-4 py-2 text-sm ${tab === 'browse' ? 'border-b-2 border-blue-500 text-blue-600 dark:text-blue-400' : 'text-slate-500'}`}>
            کاتالوگ منابع ایرانی ({SEED_FEEDS.length})
          </button>
          <button onClick={() => setTab('custom')} className={`px-4 py-2 text-sm ${tab === 'custom' ? 'border-b-2 border-blue-500 text-blue-600 dark:text-blue-400' : 'text-slate-500'}`}>
            افزودن نشانی دلخواه
          </button>
        </div>
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center gap-2">
              <div className="flex-1 relative">
                ...
                <input value={query} onChange={e => setQuery(e.target.value)} placeholder="جستجو در نام، دسته یا نوع..."
                  className="w-full bg-slate-100 dark:bg-slate-800 rounded-lg pr-9 pl-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              ...
              <button onClick={addAllIrani} disabled={loading} className="text-sm px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">افزودن کل کاتالوگ</button>
            </div>
                    ...
                    {list.map((f, i) => {
                      ...
                      return (
                        <button key={`${f.url}__${f.category}__${i}`} onClick={() => toggle(f.url)}
                          className={`flex items-center gap-2 p-2 rounded-lg text-right text-sm border ${isSel ? 'bg-blue-50 dark:bg-blue-950/40 border-blue-400' : 'border-transparent hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                          <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${isSel ? 'bg-blue-600 border-blue-600' : 'border-slate-300 dark:border-slate-600'}`}>
        ...
        {tab === 'custom' && (
          <div className="p-6 flex-1">
            ...
            <input dir="ltr" value={customUrl} onChange={e => setCustomUrl(e.target.value)} placeholder="https://example.com/feed.xml"
              className="w-full bg-slate-100 dark:bg-slate-800 rounded-lg px-3 py-2 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            <button onClick={submitCustom} disabled={loading || !customUrl}
              className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-lg py-2 flex items-center justify-center gap-2">
              {loading && <Loader2 className="w-4 h-4 animate-spin" />}
              افزودن
            </button>
          </div>
