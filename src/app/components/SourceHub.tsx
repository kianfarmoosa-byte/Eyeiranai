...
export function SourceHub({ open, onClose, feeds, onChanged }: Props) {
  ...
  const importOpmlConfirm = async () => {
    ...
    try {
      ...
      setMsg({ kind: "ok", text: `${toFa(opmlPreview.add.length)} فید جدید افزوده شد. ${toFa(opmlPreview.skip.length)} مورد تکراری بود.` });
          ...
          {([
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex items-center gap-2 px-4 py-2.5 text-sm border-b-2 transition-colors whitespace-nowrap ${tab === t.id ? "border-emerald-600 text-emerald-600 dark:text-emerald-400" : "border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200"}`}>
            ...
            <div className="space-y-4 max-w-2xl mx-auto">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none focus:border-emerald-500"
                />
              </div>
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    {has(detected.url) ? (
                      ...
                      <button onClick={addDetected} disabled={adding}
                        className="w-full py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-medium flex items-center justify-center gap-2">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="relative flex-1">
                  ...
                  <input value={catQuery} onChange={(e) => setCatQuery(e.target.value)} placeholder="جست‌وجو در کاتالوگ…"
                    className="w-full pr-9 pl-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none focus:border-emerald-500" />
                </div>
                  ...
                  return (
                    <div key={c.url} className="rounded-xl border border-slate-200 dark:border-slate-800 p-3 flex flex-col gap-2 bg-white dark:bg-slate-950">
                      ...
                      <button disabled={taken || adding} onClick={() => addOne(c.url, c.name, c.icon, c.category)}
                        className={`mt-1 w-full py-1.5 rounded-md text-sm font-medium ${taken ? "bg-slate-100 dark:bg-slate-800 text-slate-500" : "bg-emerald-600 hover:bg-emerald-700 text-white"}`}>
            ...
            <div className="space-y-4 max-w-2xl mx-auto">
              <div className="flex flex-col sm:flex-row gap-2">
                <button onClick={() => fileRef.current?.click()}
                  className="flex-1 py-3 rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-emerald-500 flex items-center justify-center gap-2">
              ...
              <div>
                ...
                <textarea value={opmlText} onChange={(e) => {
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none focus:border-emerald-500 font-mono text-xs" dir="ltr" />
              </div>
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                  <div className="px-3 py-2 bg-slate-100 dark:bg-slate-900 flex items-center justify-between text-sm">
                    ...
                    <button onClick={importOpmlConfirm} disabled={opmlPreview.add.length === 0 || opmlImporting}
                      className="px-3 py-1 rounded-md bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-xs flex items-center gap-1.5">
